/*
 * GBWUI — Firmware Update (OTA) screen (placeholder)
 *
 * Full design per DESIGN.md §5.7:
 *   No-WiFi state → show WiFi button
 *   WiFi connected → show IP, progress bar, start HTTP OTA server task
 */
...
void screen_ota_load(void)
{
    ...
    lv_label_set_text(lbl,
                      "No-WiFi: show WiFi button\n"
                      "WiFi connected: show IP (large blue),\n"
                      "progress bar, ota_srv_task on Core 0\n"
                      "XHR upload → Update.begin/write/end\n\n"
                      "— not yet implemented —");
